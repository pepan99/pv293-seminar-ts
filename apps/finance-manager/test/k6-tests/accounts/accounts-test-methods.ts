...
export class AccountsTests extends ApiBase {
    ...
    createTestAccount() {
        ...
        console.log(`User params for createTestAccount: ${JSON.stringify(this.userParams)}`);
        ...
        if (accountsRes.status === 200) {
            try {
                ...
                for (const account of accounts) {
                    http.del(`${this.accountsApiUrl}${account.id}`, null, this.userParams);
                }
            } catch (e) {
    ...
    createAccountTest() {
        ...
        if (res.status === 201) {
            ...
            check(account, {
                ...
                "Account has name field": (account) => account.name === createAccountDto.name,
                "Account has description field": (account) =>
                    account.description === createAccountDto.description,
                "Account has color field": (account) => account.color === createAccountDto.color,
                "Account has icon field": (account) => account.icon === createAccountDto.icon,
    ...
    createAccountUnauthorizedTest() {
        ...
        const payload = JSON.stringify(createAccountDto);
        const res = http.post(this.accountsApiUrl, payload, this.unauthorizedParams);
    ...
    findAllAccountsTest() {
        ...
        if (res.status === 200) {
            ...
            check(accounts, {
                ...
                "Get all accounts array is not empty": (accounts) => accounts.length > 0,
            });
        }
    }
    ...
    findOneAccountTest() {
        const res = http.get(`${this.accountsApiUrl}${this.testAccountId}`, this.userParams);
        ...
        if (res.status === 200) {
            try {
                ...
                check(account, {
                    "Get account has correct id": (account) => account.id === this.testAccountId,
                    "Get account has name field": (account) =>
                        account.name === this.testAccountName,
                    "Get account has userId field": (account) => account.userId === this.userId,
                });
            } catch (e) {
        ...
        if (res.status === 200) {
            try {
                ...
                check(account, {
                    ...
                    "Updated account has correct color field": (account) => account.color === "red",
                });
    ...
    updateAccountNotFoundTest() {
        ...
        const payload = JSON.stringify(updateAccountDto);
        const randomId = uuidv4();
        const res = http.patch(`${this.accountsApiUrl}${randomId}`, payload, this.userParams);
    ...
    removeAccountTest() {
        ...
        const createRes = http.post(this.accountsApiUrl, createPayload, this.userParams);
        ...
        const res = http.del(`${this.accountsApiUrl}${accountToDeleteId}`, null, this.userParams);

        check(res, {
            "Delete account is status 200": (r) => r.status === 200,
        });

        const getRes = http.get(`${this.accountsApiUrl}${accountToDeleteId}`, this.userParams);
    ...
    removeAccountNotFoundTest() {
        const randomId = uuidv4();
        const res = http.del(`${this.accountsApiUrl}${randomId}`, null, this.userParams);
    ...
    getAccountBalanceNotFoundTest() {
        const randomId = uuidv4();
        const res = http.get(`${this.accountsApiUrl}${randomId}/balance`, this.userParams);

        check(res, {
            "Get balance for non-existent account is status 404": (r) => r.status === 404,
        });
    }
    ...
    getTotalBalanceTest() {
        const res = http.get(`${this.accountsApiUrl}total-balance`, this.userParams);
    ...
    getTotalBalanceUnauthorizedTest() {
        const res = http.get(`${this.accountsApiUrl}total-balance`, this.unauthorizedParams);
