...
export class AccountAggregate extends AggregateRoot {
    ...
    create(
        ...
        if (!name || !accountType || !currency || !userId) {
            throw new BadRequestException("Name, account type, currency and user ID are required");
        }
    ...
    update(data: { name?: string; description?: string; icon?: string; color?: string }): void {
        ...
        if (Object.keys(changes).length > 0) {
            ...
            this.apply(
                new AccountUpdatedEvent(this._id, this._userId, data.name, this._accountType),
            );
        }
    }
