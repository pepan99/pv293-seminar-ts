...
@Injectable()
export class AccountAggregateRepository implements IAccountsAggregateRepository {
    constructor(
    ...
    async findByName(name: string, userId: string): Promise<AccountAggregate | null> {
        const accountData = await this.db
            .selectFrom("accounts")
            .selectAll()
            .where("name", "=", name)
