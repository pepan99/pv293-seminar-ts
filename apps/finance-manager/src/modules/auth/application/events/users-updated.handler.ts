...
export class UserUpdatedEventHandler implements IEventHandler<UserUpdatedMappedEvent> {
    constructor(private readonly usersRepository: UsersRepository) {}

    async handle(event: UserUpdatedMappedEvent) {
        await this.usersRepository.update(event.id, { email: event.email, name: event.name });
    }
}
