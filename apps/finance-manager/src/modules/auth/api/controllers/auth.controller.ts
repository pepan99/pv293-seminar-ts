...
export class AuthController {
    ...
    async register(@Body() createUserDto: CreateUserDto) {
        return this.commandBus.execute(
            new RegisterCommand(createUserDto.email, createUserDto.password, createUserDto.name),
        );
    }
    ...
    async refreshToken(@Body() refreshTokenDto: { refresh_token: string }) {
        return this.commandBus.execute(new RefreshTokenCommand(refreshTokenDto.refresh_token));
    }
